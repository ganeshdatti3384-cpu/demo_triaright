...
import { Search, Clock, MapPin, Building, IndianRupee, Loader2, WifiOff } from 'lucide-react';
...
const OfflineInternships = () => {
  ...
  // Fetch offline internships from backend
  useEffect(() => {
    const fetchOfflineInternships = async () => {
      try {
        setLoading(true);
        setError(null);
        
        const response = await fetch('/api/internships/ap-internships/filter/offline', {
          headers: {
        ...
        if (!response.ok) {
          ...
          if (contentType && contentType.includes('text/html')) {
            ...
            throw new Error(`Failed to fetch offline internships: ${response.status} ${response.statusText}`);
          }
        }
        ...
        if (data.success) {
          ...
          toast.success('Offline internships loaded successfully');
        } else {
          throw new Error(data.message || 'Failed to fetch internships');
        }
      } catch (err) {
        console.error('Error fetching offline internships:', err);
        ...
        toast.error('Failed to load offline internships');
      } finally {
        setLoading(false);
      }
    };

    fetchOfflineInternships();
  }, []);

  // Extract unique categories, locations, and fee types from data
  ...
  }, [internships, searchTerm, categoryFilter, feeTypeFilter, locationFilter]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
        ...
        <div className="flex justify-center items-center min-h-[400px]">
          ...
          <span className="ml-2 text-gray-600">Loading offline internships...</span>
        </div>
        <Footer />
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {filteredInternships.length === 0 && !loading && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No offline internships found matching your criteria.</p>
          </div>
        )}
      </div>

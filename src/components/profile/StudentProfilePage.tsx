...
const StudentProfilePage = () => {
  ...
  const handleArrayChange = (arrayName: string, index: number, field: string, value: string) => {
    setProfile(prev => ({
      ...prev,
      [arrayName]: prev[arrayName as keyof StudentProfile]?.map((item: any, i: number) =>
  ...
  const addArrayItem = (arrayName: string) => {
    ...
    setProfile(prev => ({
      ...prev,
      [arrayName]: [...(prev[arrayName as keyof StudentProfile] as any[]), newItem]
    }));
  };
  ...
  const removeArrayItem = (arrayName: string, index: number) => {
    setProfile(prev => ({
      ...prev,
      [arrayName]: (prev[arrayName as keyof StudentProfile] as any[]).filter((_, i) => i !== index)
    }));
  };

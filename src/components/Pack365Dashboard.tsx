    ...
    try {
      ...
      if (response.success && response.enrollments) {
        ...
        const enhancedEnrollments = await Promise.all(
          streamEnrollments.map(async (enrollment) => {
        ...
        const totalHours = enhancedEnrollments.reduce((sum: number, enrollment: StreamEnrollment) => {
          return sum + (enrollment.courses || []).reduce((courseSum, course) => 
            courseSum + (course.totalDuration || 0), 0);
        }, 0);
        
        const averageProgress = enhancedEnrollments.length > 0 
          ? enhancedEnrollments.reduce((sum: number, enrollment: StreamEnrollment) => 
              sum + (enrollment.totalWatchedPercentage || 0), 0) / enhancedEnrollments.length
          : 0;
            ...
            <Card className="bg-gradient-to-r from-blue-500 to-blue-600 text-white">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <BarChart2 className="h-12 w-12 text-blue-100" />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-r from-purple-500 to-purple-600 text-white">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <CheckCircle2 className="h-12 w-12 text-purple-100" />
                </div>
              </CardContent>
            </Card>
                    ...
                    <Card key={index} className="border-2 hover:border-blue-200 transition-colors">
                      ...
                      <CardContent>
                        <div className="space-y-4">
                          ...
                          {/* Final Exam Status (100% completion) */}
                          {enrollment.totalWatchedPercentage >= 100 && !enrollment.isExamCompleted && (
                            <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
                              <div className="flex items-center">
                                <Award className="h-5 w-5 text-green-600 mr-2" />
                                <div>
                                  <p className="text-sm font-medium text-green-800">Final Exam Available</p>
                                  <p className="text-xs text-green-700">
                                    You've completed all courses! Take the final exam.
                                  </p>
                                </div>
                              </div>
                              <Button 

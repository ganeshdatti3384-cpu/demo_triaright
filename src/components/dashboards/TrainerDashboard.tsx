  ...
  const fetchTrainerProfile = async () => {
    try {
      ...
      const transformedProfile = {
        ...
        dateOfBirth: data.personalDetails?.dateOfBirth ? new Date(data.personalDetails.dateOfBirth).toISOString().split('T')[0] : '',
        bio: data.bio || '',
        ...
        avatar: data.personalDetails?.profileImage || '',
        userId: data.userId,
...
const handleSaveProfile = async () => {
  try {
    ...
    const updatePayload = {
      personalDetails: {
        firstName: tempProfile.fullName.split(' ')[0] || '',
        lastName: tempProfile.fullName.split(' ').slice(1).join(' ') || '',
        phone: tempProfile.phone,
        dateOfBirth: tempProfile.dateOfBirth,
        gender: tempProfile.gender,
        profileImage: avatarPreview || tempProfile.avatar || '',
        address: {
          street: tempProfile.location,
    ...
    const updatedProfile = {
      ...tempProfile,
      fullName: `${data.profile.personalDetails?.firstName || ''} ${data.profile.personalDetails?.lastName || ''}`.trim(),
      phone: data.profile.personalDetails?.phone || tempProfile.phone,
      location: data.profile.personalDetails?.address?.street || tempProfile.location,
      city: data.profile.personalDetails?.address?.city || tempProfile.city,
      state: data.profile.personalDetails?.address?.state || tempProfile.state,
      pincode: data.profile.personalDetails?.address?.pincode || tempProfile.pincode,
      bio: data.profile.bio || tempProfile.bio,
      expertise: data.profile.expertise || tempProfile.expertise,
      gender: data.profile.personalDetails?.gender || tempProfile.gender,
      dateOfBirth: data.profile.personalDetails?.dateOfBirth
        ? new Date(data.profile.personalDetails.dateOfBirth).toISOString().split('T')[0]
        : tempProfile.dateOfBirth,
      avatar: avatarPreview || data.profile.personalDetails?.profileImage || tempProfile.avatar,
      education: data.profile.education || tempProfile.education,
      experience: data.profile.experience || tempProfile.experience,
      certifications: data.profile.certifications || tempProfile.certifications
    };
...
const handleExpertiseChange = (value: string) => {
  ...
  setTempProfile(prev => ({ ...prev, expertise: expertiseArray }));
};
...
const removeEducation = (index: number) => {
  const updated = tempProfile.education.filter((_, i) => i !== index);
  setTempProfile(prev => ({ ...prev, education: updated }));
};
...
const removeCertification = (index: number) => {
  const updated = tempProfile.certifications.filter((_, i) => i !== index);
  setTempProfile(prev => ({ ...prev, certifications: updated }));
};
...
const updateExperience = (
  ...
  setTempProfile(prev => ({ ...prev, experience: updated }));
};

const removeExperience = (index: number) => {
  const updated = tempProfile.experience.filter((_, i) => i !== index);
  setTempProfile(prev => ({ ...prev, experience: updated }));
};
                ...
                <div className="space-y-4">
                  {sessions.filter(s => {
                    ...
                    sessions.filter(s => {
                      ...
                      <div key={session.id} className="p-3 rounded-lg border hover:bg-orange-50/30 transition-colors group">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">
                              {session.batch} • {new Date(session.scheduledDate).toLocaleDateString()}
                            </p>
  ...
  <div className="space-y-4">
    ...
    <div className="space-y-2">
      ...
      <Input
        id="expertise"
        name="expertise"
        value={tempProfile.expertise?.join(', ') || ''}
    ...
    <div className="space-y-4 border-t pt-4 mt-6">
      ...
      {tempProfile.education?.map((edu: any, idx: number) => (
        <div key={idx} className="border p-4 rounded-lg space-y-3 bg-gray-50">
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-700">Education #{idx + 1}</span>
            <Button variant="destructive" size="sm" onClick={() => removeEducation(idx)} type="button">
    ...
    <div className="space-y-4 border-t pt-4 mt-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={addExperience} variant="outline" size="sm" type="button">
          <Plus className="h-4 w-4 mr-1" />
          Add Experience
        </Button>
      </div>
      {tempProfile.experience?.map((exp: any, idx: number) => (
        <div key={idx} className="border p-4 rounded-lg space-y-3 bg-gray-50">
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-700">Experience #{idx + 1}</span>
            <Button variant="destructive" size="sm" onClick={() => removeExperience(idx)} type="button">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
          <Input
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded"
            />
            <Label htmlFor={`currently-working-${idx}`} className="text-sm cursor-pointer">
    ...
    <div className="space-y-4 border-t pt-4 mt-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={addCertification} variant="outline" size="sm" type="button">
          <Plus className="h-4 w-4 mr-1" />
          Add Certification
        </Button>
      </div>
      {tempProfile.certifications?.map((cert: any, idx: number) => (
        <div key={idx} className="border p-4 rounded-lg space-y-3 bg-gray-50">
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-700">Certification #{idx + 1}</span>
            <Button variant="destructive" size="sm" onClick={() => removeCertification(idx)} type="button">
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
          <Input
...
</Card><Card className="shadow-lg border-0 overflow-hidden mt-6">
              <CardHeader className="bg-gradient-to-r from-gray-50 to-white border-b">
                <CardTitle className="text-lg font-bold flex items-center gap-2">
...
<div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-gray-100 p-4 md:p-6">
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">
            Trainer <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Dashboard</span>
          </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-1">
              {navItems.map((item) => {
                ...
                return (
                 ...
                 className={`relative min-w-[190px] sm:min-w-[210px] flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-300 group ${
  isActive
